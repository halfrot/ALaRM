...
class PPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        if not isinstance(tokenizer, (PreTrainedTokenizerBase)):
            raise ValueError(
                ...
                f"PreTrainedTokenizerFast, got {type(tokenizer)}"
            )
    ...
    def prepare_model_inputs(self, queries: torch.Tensor, responses: torch.Tensor):
        if self.is_encoder_decoder:
            input_data = self.data_collator(
                [{
                    "input_ids": q,
                    "attention_mask": torch.ones_like(q)
                } for q in queries]
            ).to(self.current_device)
    ...
    def compute_rewards(
        ...
        for score, logprob, ref_logprob, mask in zip(scores, logprobs, ref_logprobs, masks):
            ...
            # for fine-grained rlhf
            score = torch.cat((torch.zeros(non_score_reward.size(0) - score.size(0)).to(self.current_device), score))
            reward += score
    ...
    def record_step_stats(self, kl_coef: float, **data):
        ...
        # for fine-grained rlhf
        mean_scores = torch.stack([s.mean() for s in data["scores"]]).mean()
        std_scores = torch.stack([s.mean() for s in data["scores"]]).std()
    ...
    def log_stats(
        ...
        if self.accelerator.is_main_process:
            ...
            if "query" not in batch.keys() and "response" not in batch.keys():
                ...
                logs.update({
                    "game_log": wandb.Table(columns=["query", "response", "reward"], rows=table_rows)
                })
